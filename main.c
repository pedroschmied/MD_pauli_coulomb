...
//------Datos de la simulación
	...
	int correlacion = N * 2, pasos = 10 * N * correlacion, pre_termalizacion = 30000000, termalizacion = 100000;

	double Temp = T0, dT = -0.025, Tf = 0.05 + 2.0 * dT;
...
//-------Metrópolis
	...
	double ax =  0.11209413133659944 , bx =  0.02599064113173173 , ap =  1.0310625344524857 , bp =  0.009224327807405897 ;//parametros del ajuste para calcular los dx y dp
	
	dp = sqrt(T0) * ap + bp;
	dx = sqrt(T0) * ax + bx;
	dx = dx;// * dep_rho * dep_rho;
...
//--------Loop de temperaturas
	...
	FILE * fp;
	char filename[500];
	sprintf (filename,"/home/pedro/Desktop/Metropolis_Pauli/Datos/barrido_rho_%.2lf.txt", rho);
	fp = fopen(filename, "w");
	for (t = 0; t < tfinal; t++)
	{		
		fprintf(fp, "%lf\t", T0);
